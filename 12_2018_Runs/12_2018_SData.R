...
for(i in 1:2){
  sir_reference[[i]] <-  HUMPBACK.SIR(file_name = paste0("Reference/Reference", c("","prior")[i]),
                                      n_resamples = 10000,
...
plot_density(sir_reference,  file_name = "Reference/Reference", multiple_sirs = TRUE, lower = c(NA, 22000, 0, NA, 20000, 0, 0.35, 0.6), upper = c(NA, 40000, 2000, NA, 28000, 0.1, NA, NA))
...
sir_sdata_1 <- list()
for(i in 1:2){
  sir_sdata_1[[i]] <-  HUMPBACK.SIR(file_name = paste0("SData 1/SData 1", c("","prior")[i]),
                                      n_resamples = 10000,
                                      priors = make_prior_list(r_max = make_prior(runif, 0, 0.118),
                                                               N_obs = make_prior(runif, 500, 40000)),
                                      catch_multipliers = make_multiplier_list(
