...
for(i in 1:2){
  sir_sdata_3[[i]] <-  HUMPBACK.SIR(file_name = paste0(file_name, c("","prior")[i]),
                                    ...
                                    premodern_catch_data = merge(PreModern.Catch.Min, PreModern.Catch.Max, by = "Year", all = T),
                                    control = sir_control(threshold = 1e-34, progress_bar = TRUE),
                                    realized_prior = ifelse(i == 1, "FALSE", "TRUE"))
}
...
for(i in 1:2){
  sir_sdata_4[[i]] <-  HUMPBACK.SIR(file_name = paste0(file_name, c("","prior")[i]),
                                    ...
                                    premodern_catch_data = merge(PreModern.Catch.Min, PreModern.Catch.Max, by = "Year", all = T),
                                    control = sir_control(threshold = 0.1 * 1e-16, progress_bar = TRUE),
                                    realized_prior = ifelse(i == 1, "FALSE", "TRUE"))
}
...
for(i in 1:2){
  sir_catch_1[[i]] <-  HUMPBACK.SIR(file_name = paste0(file_name, c("","prior")[i]),
                                    ...
                                    control = sir_control(threshold = 1e-26, progress_bar = TRUE),
                                    realized_prior = ifelse(i == 1, "FALSE", "TRUE"))
}
...
# Scatch 4 - Liberal
Core.1901.1920 <- Core.Catches2[Core.Catches2$Year %in% c(1830:1920), ]
Core.1920.after <- Core.Catches2[Core.Catches2$Year > 1920, ]
Core.Catches.Reorg4 <- as.matrix(merge(Core.1901.1920, Core.1920.after, by = "Year", all = TRUE))
...
for(i in 1:2){
  sir_catch_4[[i]] <-  HUMPBACK.SIR(file_name = paste0(file_name, c("","prior")[i]),
                                    ...
                                    premodern_catch_data = merge(PreModern.Catch.Min, PreModern.Catch.Max, by = "Year", all = T),
                                    control = sir_control(threshold = 1e-22, progress_bar = TRUE),
                                    realized_prior = ifelse(i == 1, "FALSE", "TRUE"))
}
...
for(i in 1:2){
  sir_msyr_2[[i]] <-  HUMPBACK.SIR(file_name = paste0(file_name, c("","prior")[i]),
                                   ...
                                   premodern_catch_data = merge(PreModern.Catch.Min, PreModern.Catch.Max, by = "Year", all = T),
                                   control = sir_control(threshold = 4e-25, progress_bar = TRUE),
                                   realized_prior = ifelse(i == 1, "FALSE", "TRUE"))
}
